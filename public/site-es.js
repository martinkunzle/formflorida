...
function showStep(i){current=Math.max(0,Math.min(steps.length-1,i));form.dataset.currentStep=String(current+1);steps.forEach((s,idx)=>{const active=idx===current;s.classList.toggle('active',active);if(active)s.setAttribute('aria-current','step');else s.removeAttribute('aria-current');});label.textContent=`Paso ${current+1} de ${steps.length}`;nameEl.textContent=steps[current].dataset.name;bar.style.width=`${((current+1)/steps.length)*100}%`;back.hidden=current===0;next.hidden=current===steps.length-1;checkout.hidden=current!==steps.length-1;if(current===steps.length-1) renderReview();steps[current].querySelector('input,select,textarea')?.focus({preventScroll:true});}
