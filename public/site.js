...
;(()=>{document.querySelectorAll('.annual-subscription-form').forEach(form=>{form.addEventListener('submit',async e=>{e.preventDefault();const btn=form.querySelector('button[type="submit"]'),msg=form.querySelector('.annual-sub-msg');if(!form.reportValidity())return;btn.disabled=true;btn.textContent='Opening secure subscription checkout…';msg.classList.remove('show');try{const payload=Object.fromEntries(new FormData(form).entries());const r=await fetch('/api/create-annual-subscription',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});const d=await r.json();if(!r.ok||!d.url)throw new Error(d.error||'Unable to start subscription checkout.');window.location.assign(d.url)}catch(err){msg.textContent=err.message||'Unable to start subscription checkout.';msg.classList.add('show');btn.disabled=false;btn.textContent='Enroll in Annual Plan';}})});})();
